...
const store = globalErrorHandler(async (req, res) => {
  let { name, sku, category, quantity, price, description } = req.body;

  if (!name || !category || !quantity || !price || !description) {
    res.status(StatusCodes.BAD_REQUEST);
    throw new Error(
      "name, category, quantity, price, and description required"
    );
  }
...
const index = globalErrorHandler(async (req, res) => {
  const products = await ProductModel.find({ userId: req.user._id }).sort(
    "-createdAt"
  );
